...
@Component({
  selector: 'app-analyze-documents',
  standalone: true,
  imports: [CommonModule, SidebarComponent, HeaderComponent],
  templateUrl: './analyze-documents.component.html',
})
export class AnalyzeDocumentsComponent {
...
uploadComplete: boolean = false;
 // Add this in your component
  ...
  onDrop(event: DragEvent): void {
    ...
    if (files && files.length > 0) {
      ...
      if (this.isValidFile(file)) {
        ...
        this.errorMessage = 'Please upload a valid file (PDF, JPEG, or PNG - max 10MB).';
      }
    }
  }
  ...
  analyzeDocument(): void {
    ...
    this.analyzeDocumentsService.analyzeDocuments(formData).subscribe({
      next: (event) => {
        if (event.done) {
  ...
  getFormattedResults(): string {
    ...
    if (this.analysisResults['rawOutput']) {
      if (typeof this.analysisResults['rawOutput'] === 'string') {
        ...
        return this.analysisResults['rawOutput'].summary+ '\n \n \n' + 'Recommendation :'+ this.analysisResults['rawOutput'].recommendations;
